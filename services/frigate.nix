...
let
  ...
  configuration = {
    ...
    cameras = mapAttrs
      (key: value: {
        ffmpeg.inputs = [{
          path = "rtsp://${value.user}:${value.password}@${key}.home.gustafson.me:554";
        ...
        motion.mask = value.motionMask;
      } // (if (value ? zones) then {
        zones = mapAttrs (key: value: { coordinates = value; }) value.zones;
      } else { }) // (if (value ? objectMasks) then {
        objects.filters = {
          car.mask = value.objectMasks;
          person.mask = value.objectMasks;
        };
      } else { }))
      cameras;
    go2rtc.streams = mapAttrs
      (key: value: [ "rtsp://${value.user}:${value.password}@${key}.home.gustafson.me:554/cam/realmonitor?channel=1&subtype=0" ])
      cameras;
  };
in
{ config, pkgs, ... }:
