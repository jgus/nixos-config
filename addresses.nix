...
let
  ...
  # Parse a hex string (1-2 chars) to integer
  hexToInt = hex:
    let
      chars = lib.stringToCharacters (lib.strings.toLower hex);
      charToVal = c:
    ...
    in
    if length chars == 1 then charToVal (head chars)
    else (charToVal (head chars)) * 16 + (charToVal (lib.lists.last chars));
  ...
  macToIp6 = mac:
    let
      ...
      prefixBase = lib.strings.removeSuffix "::" network.prefix6;
    in
