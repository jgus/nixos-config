...
in
{
  homelab.services = lib.homelab.recursiveUpdates (map
    (n:
      let
  ...
  sops = lib.mkIf (config.homelab.services.pihole-1.enable || config.homelab.services.pihole-2.enable || config.homelab.services.pihole-3.enable) {
    secrets.pihole = { };
  };
}
